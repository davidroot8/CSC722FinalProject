...
def list_directory_contents(directory):
    try:
        ...
        print("Permission denied to access directory '{}'.".format(directory))
...
#     model.vgg.load_state_dict(vgg_weights)
    ...
    pathOS = os.path.join(current_directory, 'SSD_ResNet_Pytorch', 'archive', 'imagenetMini', 'train')
    print("Absolute path:", os.path.abspath(pathOS))
    image_sets = [['2007', 'trainval'], ['2012', 'trainval']]

    list_directory_contents(pathOS)
    list_directory_contents(r".\archive\imagenetMini\train")
    list_directory_contents('./archive/imagenetMini/train')
    list_directory_contents(os.getcwd())

    dataset = ImageFolder(root=pathOS, transform=transform)
